...
- **Implementation deviation (Task 4 gesture detection):** the planned `UITapGestureRecognizer(numberOfTouchesRequired: 4)` does **not** work inside SDL's directly-owned `UIWindow` — instrumentation (KVO on `state`, a delegate, a file-write in the action) showed the recognizer reaches `.recognized` but its target-action never fires, because no `UIViewController`-hosted scene backs the window. Plain responder-chain touch delivery (`touchesBegan/Moved/Ended`) works reliably, so the shipped code detects the four-finger tap by counting touches in `touchesBegan`/`endTouches` (`summonTouches`/`summonArmed`) instead. Functionally equivalent; the summon UITest passes. A follow-up guard fix (commit `1faf6c7`) snapshots `keyboardActive` before the tracking call so a dismiss-tap's touches can't fall through into movement.
- **UITest coverage limit (Task 4):** the "second four-finger tap dismisses" path can't be driven by XCUITest once the real iOS keyboard covers the screen (overlapping full-screen elements), so the UITest exercises dismiss via the keyboard's Return key; tap-to-dismiss is covered by on-device Step 7.

...
- [A CLI xcodebuild sharing DerivedData with an open Xcode races its preview builds](cli-builds-race-xcodes-previews.md) — presents as a locked build.db on one side and a SwiftData launch crash in the canvas on the other; give CLI runs their own `-derivedDataPath`

...
- [A green layout suite says nothing about the composed screen](geometry-tests-cannot-see-the-screen.md) — five visual defects shipped under all-green geometry tests; `App/Sources/UI/ShelfPreviews.swift` is where a human eye plugs in, so look at the render before trusting the numbers
- [The preview host crashes in App.main() — gate app startup, don't share DerivedData](cli-builds-race-xcodes-previews.md) — SwiftData under XOJIT dies at preview boot, so `WaddleApp.init` returns empty under `XCODE_RUNNING_FOR_PREVIEWS`; CLI builds get their own `-derivedDataPath`; and count the crash reports before believing a coincidence
- [App Review notes that pre-empt one guideline can invite another](app-review-notes-argue-the-wrong-guideline.md) — the 4.3(a) rejection: naming "comparable approved apps" to defend 5.2 handed a spam reviewer the comparison; answer in Resolution Center, not with a new build
